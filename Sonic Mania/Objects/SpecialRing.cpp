...
void SpecialRing_Draw()
{
    ...
    if (entity->state == SpecialRing_State_Warp) {
        ...
        RSDK.Init3DScene(SpecialRing->sceneIndex);
        if (entity->enabled)
            RSDK.SetupMesh(SpecialRing->modelIndex, SpecialRing->sceneIndex, 6, &entity->matrix2, &entity->matrix3, 0xF0F000);
        else
            RSDK.SetupMesh(SpecialRing->modelIndex, SpecialRing->sceneIndex, 4, &entity->matrix2, &entity->matrix3, 0x609090);
...
void SpecialRing_State_Warp()
{
    ...
    if (SaveGame->saveRAM[28] == 127 || !entity->id) {
        RSDK.ResetEntityPtr(entity, 0, 0);
    }
    else {
...
void SpecialRing_State_Normal()
{
    ...
    if (entity->scale.x >= 320)
        ...
        entity->scale.x = entity->scale.x + ((360 - entity->scale.x) >> 5);
