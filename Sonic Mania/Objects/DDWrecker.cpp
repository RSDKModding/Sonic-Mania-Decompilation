...
void DDWrecker_State_Init()
{
    ...
    if (++entity->timer >= 8) {
        ...
        DDWrecker->field_C             = entity->position.x + ((32 - screen->centerX) << 16);
        DDWrecker->field_10            = entity->position.x + ((screen->centerX - 32) << 16);
        DDWrecker->field_14            = entity->position.y - 0xC00000;
...
void DDWrecker_State_InitChildren()
{
    ...
    if (entity->timer) {
        entity->timer++;
        if (entity->timer == 60) {
            EntityDDWrecker *child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 1);
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)3);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            child->position.y += 0x400000;

            child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 2);
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)3);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            child->position.y += 0x400000;

            child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 4);
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)3);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            child->position.y += 0x400000;

            child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 5);
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)3);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            child->position.y += 0x400000;

            child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 3); // main
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)4);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            ...
            child->bodyA      = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 6);
            child->bodyB      = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 7);
            child->state      = DDWrecker_State_Assemble;

            child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 6); // bodyA
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)1);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            child->position.y += 0x400000;
            ...
            child = (EntityDDWrecker *)RSDK.GetEntityByID(RSDK_sceneInfo->entitySlot + 7); // bodyB
            RSDK.DestroyEntity(child, DDWrecker->objectID, (void *)2);
            child->position.x = entity->position.x;
            child->position.y = entity->position.y;
            child->position.y += 0x400000;
            child->slots[0]  = RSDK_sceneInfo->entitySlot + 1;
        ...
        if (RSDK.GetEntityByID(SLOT_PLAYER1)->position.x > entity->position.x) {
            Music_Unknown9(3, 0.0125);
            ++entity->timer;
        }
    }
}
...
void DDWrecker_State_Unknown2()
{
    ...
    if (entity->timer2 <= 0) {
        ...
        EntityDDWrecker *child = 0;
        while (RSDK.GetActiveObjects(DDWrecker->objectID, (Entity **)&child)) {
...
void DDWrecker_State_Unknown4()
{
    ...
    if (entity->angle3 < 0x200) {
        ...
        if (entity->position.x < DDWrecker->field_4)
            entity->position.x = DDWrecker->field_4;
    }
    entity->angle3 += 4;
    entity->angle3 &= 0x3FF;
    entity->angle += 2;
    ...
    if (entity->angle3 >= 768) {
        if (--entity->timer <= 0) {
            EntityDDWrecker *child = (EntityDDWrecker *)RSDK.GetEntityByID(entity->slots[2]);
            ...
            if (entity == entity->bodyB)
                child->angle3 += 0x200;
            entity->state            = 0;
            entity->bodyA->lateState = DDWrecker_LateState_Unknown2;
            entity->bodyB->lateState = DDWrecker_LateState_Unknown2;
            RSDK.PlaySFX(DDWrecker->sfx_Sharp, 0, 255);
        }
        else {
            ...
            if (entity->timer == 2) {
                if (entity->bodyA->lateState == DDWrecker_LateState_Unknown1)
                    entity->bodyA->lateState = DDWrecker_LateState_Unknown2;
                else
                    entity->bodyA->lateState = DDWrecker_LateState_Unknown1;

                if (entity->bodyB->lateState == DDWrecker_LateState_Unknown1)
                    entity->bodyB->lateState = DDWrecker_LateState_Unknown2;
                else
                    entity->bodyB->lateState = DDWrecker_LateState_Unknown1;
                RSDK.PlaySFX(DDWrecker->sfx_Sharp, 0, 255);
            }
        }
    }
...
void DDWrecker_State_Unknown5()
{
    ...
    if (entity->angle3 > 511) {
        entity->position.x += entity->velocity.x;
        if (entity->position.x > DDWrecker->field_8)
            entity->position.x = DDWrecker->field_8;
    }
    entity->angle3 += 4;
    entity->angle3 &= 0x3FF;
    entity->angle += 2;
...
void DDWrecker_State_Unknown6()
{
    ...
    else if (entity->position.y > entity->startPos.y) {
        entity->velocity.y = 0;
        entity->position.y = entity->startPos.y;
    }
...
void DDWrecker_State_Unknown9()
{
    ...
    if (entity->startPos.y < DDWrecker->field_14) {
        entity->startPos.y = DDWrecker->field_14;
        entity->velocity.y = 0;
    }
    ...
    entity->timer4 = 8;
    if (entity->position.x <= DDWrecker->field_C || entity->position.x >= DDWrecker->field_10) {
        entity->position.x = DDWrecker->field_C;
        entity->velocity.x = 0;
        if (entity->startPos.y <= DDWrecker->field_14)
    ...
    if (entity->velocity.y > 0) {
        if (RSDK.ObjectTileCollision(entity, Zone->fgLayers, 0, 0, 0, 0x180000, true)) {
            ...
            if (entity->timer - 1) {
                ...
                if (entity->position.x - (DDWrecker->field_C + 0x100000) > 0)
...
void DDWrecker_Spin()
{
    ...
    EntityDDWrecker *child  = NULL;
    for (int i = 0; i < 6; ++i) {
        child             = (EntityDDWrecker *)RSDK.GetEntityByID(entity->slots[i]);
        child->position.x = DDWrecker->array1[i] * RSDK.Sin1024(entity->angle3) * entity->arcOffset;
        child->position.y = DDWrecker->array1[i] * RSDK.Cos1024(entity->angle3) * entity->arcOffset;
        child->position.x += entity->position.x;
        child->position.y += entity->position.y;
...
void DDWrecker_Spin2()
{
    ...
    for (int i = 0; i < 6; ++i) {
        child             = (EntityDDWrecker *)RSDK.GetEntityByID(entity->slots[i]);
        child->position.x = DDWrecker->array2[i] * RSDK.Sin1024(RSDK.Sin1024(entity->timer2) >> 2) * entity->arcOffset;
        child->position.y = DDWrecker->array2[i] * RSDK.Cos1024(RSDK.Sin1024(entity->timer2) >> 2) * entity->arcOffset;
        child->position.x += entity->position.x;
        child->position.y += entity->position.y;
    }
...
void DDWrecker_State_Die()
{
    ...
    if (!(Zone->timer % 3)) {
        ...
        if (Zone->timer & 4) {
            Entity *explosion = RSDK.SpawnEntity(Explosion->objectID, (void *)((RSDK.Rand(0, 256) > 192) + 2),
    ...
    if (++entity->timer == 80) {
        int cnt                = 0;
        EntityDDWrecker *child = 0;
        while (RSDK.GetActiveObjects(DDWrecker->objectID, (Entity **)&child)) ++cnt;
        if (cnt != 1) {
            RSDK.DestroyEntity(entity, 0, 0);
        }
        else {
            ...
            Player_GiveScore((EntityPlayer *)RSDK.GetEntityByID(SLOT_PLAYER1), 1000);
        }
    }
}
void DDWrecker_State_SpawnSignpost()
{
    ...
    if (++entity->timer == 48) {
        ...
        while (RSDK.GetObjects(SignPost->objectID, (Entity **)&signPost)) {
