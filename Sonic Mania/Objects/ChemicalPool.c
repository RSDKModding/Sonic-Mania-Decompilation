...
void ChemicalPool_SpawnDebris(int x, int y)
{
    ...
    for (int i = 0; i < 6; ++i) {
        EntityDebris *debris = (EntityDebris *)RSDK.CreateEntity(Debris->objectID, 0, x + RSDK.Random(-0x80000, 0x80000, &Zone->randKey),
                                                                 y + RSDK.Random(0x40000, 0x140000, &Zone->randKey));
