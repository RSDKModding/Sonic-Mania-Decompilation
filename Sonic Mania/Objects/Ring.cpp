...
void Ring_State_Attract()
{
    ...
    Ring_Collect();
    //entity->animData.frameID = Zone[30];
}
void Ring_State_Big()
{
    EntityRing *entity = (EntityRing *)RSDK_sceneInfo->entity;
...
{
    EntityRing *entity = (EntityRing *)RSDK_sceneInfo->entity;
    //entity->offset.x   = entity->amplitude.x * RSDK.Cos1024(entity->moveSpeed * Zone->timer + 4 * entity->angle) + entity->position.x;
    //entity->offset.y = entity->amplitude.y * RSDK.Sin1024(entity->moveSpeed * Zone->timer + 4 * entity->angle) + entity->position.y;
    Ring_Collect();
...
{
    EntityRing *entity = (EntityRing *)RSDK_sceneInfo->entity;
    //entity->offset.x   = entity->amplitude.x * RSDK.Sin1024(entity->moveSpeed * Zone->timer) + entity->position.x;
    //entity->offset.y   = entity->amplitude.y * RSDK.Sin1024(entity->moveSpeed * Zone->timer) + entity->position.y;
    Ring_Collect();
...
void Ring_State_Track()
{
    ...
    Entity* track = RSDK.GetObjectByID(entity->moveSpeed);
...
byte Ring_CheckPlatformCollisions(EntityPlatform *platform)
{
    ...
    if (platform->state != Platform_State_Falling && platform->state != Platform_State_OffScreenReset) {
        ...
        switch (platform->collisionType) {
            ...
            case 0xE:
            case 0xF:
                boxResult = RSDK.CheckObjectCollisionBox(platform, RSDK.GetHitbox(&platform->animData, 1), entity, &Ring->hitbox, 1);
                break;
            case 2u:
                boxResult = 0;
                //if (RSDK.CheckObjectCollisionTouchBox(platform, &platform->hitbox, entity, &Obj_Ring->hitbox)
                //    && entity->collisionLayers & Zone->Move_ID) {
                //    TileLayer* moveLayer             = RSDK.GetSceneLayerPtr(Zone->MoveLayer);
...
void Ring_Serialize()
{
    RSDK.SetEditableVar(ATTRIBUTE_VAR, "type", Ring->objectID, offsetof(EntityRing, ringType));
    RSDK.SetEditableVar(ATTRIBUTE_VAR, "planeFilter", Ring->objectID, offsetof(EntityRing, planeFilter));
    RSDK.SetEditableVar(ATTRIBUTE_VAR, "moveType", Ring->objectID, offsetof(EntityRing, moveType));
    RSDK.SetEditableVar(ATTRIBUTE_VECTOR2, "amplitude", Ring->objectID, offsetof(EntityRing, amplitude));
    RSDK.SetEditableVar(ATTRIBUTE_VAR, "speed", Ring->objectID, offsetof(EntityRing, moveSpeed));
    RSDK.SetEditableVar(ATTRIBUTE_S32, "angle", Ring->objectID, offsetof(EntityRing, angle));
}
