...
void Player_LateUpdate(void)
{
    ...
    if (entity->hurtFlag) {
        ...
        switch (entity->hurtFlag) {
            default: break;
            case HURTFLAG_HURT: RSDK.PlaySfx(Player->sfx_Hurt, 0, 255);
            case HURTFLAG_DIE:
    ...
    if (entity->playerID != 1 || (globals->playerID & 0xFF) != ((globals->playerID >> 8) & 0xFF) || globals->gameMode == MODE_TIMEATTACK) {
        ...
        switch (entity->characterID) {
            case ID_SONIC:
                ...
                for (int c = 0; c < 6; ++c) RSDK.SetPaletteEntry(0, c + 0x40, colourStore[c]);
                break;
            case ID_TAILS:
                for (int c = 0; c < 6; ++c) {
                ...
                for (int c = 0; c < 6; ++c) RSDK.SetPaletteEntry(0, c + 0x46, colourStore[c]);
                break;
            case ID_KNUCKLES:
                for (int c = 0; c < 6; ++c) {
                ...
                for (int c = 0; c < 6; ++c) RSDK.SetPaletteEntry(0, c + 0x50, colourStore[c]);
                break;
#if RETRO_USE_PLUS
            case ID_MIGHTY:
                for (int c = 0; c < 6; ++c) {
                ...
                for (int c = 0; c < 6; ++c) RSDK.SetPaletteEntry(0, c + 96, colourStore[c]);
                break;
            case ID_RAY:
                for (int c = 0; c < 6; ++c) {
                ...
                for (int c = 0; c < 6; ++c) RSDK.SetPaletteEntry(0, c + 113, colourStore[c]);
                break;
#endif
        }
    }
...
void Player_P2JumpBackIn(void)
{
    ...
    if (player1->objectID == Player->objectID && (player1->drawOrder != 2 || entity->drawOrder != 2)) {
        int rx = abs((entity->position.x >> 0x10) - RSDK_screens->position.x - RSDK_screens->centerX);
        int ry = abs((entity->position.y >> 0x10) - RSDK_screens->position.y - RSDK_screens->centerY);
            ...
            if (entity->pushing > -3 && entity->pushing < 3) {
                if (entity->groundVel || (entity->angle >= 32 && entity->angle <= 224 && !entity->invertGravity)) {
                    ...
                    switch (entity->flailing - 1) {
                        ...
                        case 15:
                        case 23:
                            if (entity->direction && (entity->characterID != ID_SONIC || entity->superState != SUPERSTATE_SUPER) && !entity->isChibi) {
                                RSDK.SetSpriteAnimation(entity->spriteIndex, ANI_BALANCE2, &entity->playerAnimator, false, 0);
                            }
                            else {
                                entity->direction = FLIP_NONE;
...
void Player_EndFlyJumpIn(EntityPlayer *thisEntity, EntityPlayer *player)
{
    ...
    if (!SizeLaser || thisEntity->state == Player_State_FlyIn) {
        ...
        thisEntity->entPtr        = SizeLaser_P2JumpInResize;
        thisEntity->maxGlideSpeed = entPtr->position.x - thisEntity->position.x;
        thisEntity->field_1F8     = entPtr->position.y - thisEntity->position.y;
