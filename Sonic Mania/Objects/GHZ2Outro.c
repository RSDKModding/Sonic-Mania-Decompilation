...
{
    void *states_GHZ2[3]  = { GHZ2Outro_CutsceneState1_Unknown1, GHZ2Outro_CutsceneState1_Unknown2, NULL };
    void *states_Outro[7] = { GHZ2Outro_CutsceneState2_Unknown1,
                              GHZ2Outro_CutsceneState2_Unknown2,
                              GHZ2Outro_CutsceneState2_Unknown3,
                              GHZ2Outro_CutsceneState2_Unknown4,
                              GHZ2Outro_CutsceneState2_Unknown5,
                              GHZ2Outro_CutsceneState2_Unknown6,
                              NULL };
...
bool32 GHZ2Outro_CutsceneState2_Unknown1(EntityCutsceneSeq *host)
{
    ...
    if (host->timer >= 8) {
        ...
        // deRobot->state  = (int)DERobot_Unknown43;
            ...
            if (host->timer >= 60) {
                ...
                if (player->onGround && player->groundedStore) {
                    ...
                    if (player->characterID == ID_TAILS)
                        RSDK.SetSpriteAnimation(player->spriteIndex, ANI_BALANCE2, &player->playerAnimData, false, 0);
                    else
                        RSDK.SetSpriteAnimation(player->spriteIndex, ANI_IDLE, &player->playerAnimData, false, 0);
                }
            }
        }
...
bool32 GHZ2Outro_CutsceneState2_Unknown3(EntityCutsceneSeq *host)
{
    ...
    if (host->timer == 160) {
        /*foreach_active(DERobot, robot)
        {
            if (robot->aniID == 2 && robot->frameID == 2) {
                robot->currentState = (int)DERobot_Unknown23;
            }
            robot->currentState = (int)DERobot_Unknown23;
        }
    ...
    if (host->timer == 240) {
        ...
        eggman->animID = 2;
        Music_SetMusicTrack("RubyPresence.ogg", 5, 198457);
        Music_TransitionTrack(TRACK_EGGMAN1, 0.025);
    }

    if (host->timer == 272) {
        EntityCutsceneHBH *shinobi = CutsceneHBH_GetEntity(HBH_SHINOBI);
        if (shinobi)
    ...
    if (ruby) {
        if (ruby->state = PhantomRuby_Unknown5) {
            if (player2->objectID == Player->objectID && player2->characterID == ID_TAILS) {
                ...
                RSDK.SetSpriteAnimation(player2->spriteIndex, ANI_SKID, &player2->playerAnimData, false, 0);
            }
            return true;
        }
    }
        ...
        if (fxRuby->flag) {
            ...
            if (host->timer >= host->field_68 + 52) {
                ...
                for (int angle = 0; angle < 0x80; angle += 0x10) {
                    ...
                    int valX = (deRobot->position.x - 0x400000) - player->position.x;
                    int valY = (deRobot->position.y - (0xA00000 + 944)) - player->position.y;
