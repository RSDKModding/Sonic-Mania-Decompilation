...
void Ring_Create(void *data)
{
    ...
    if (data) {
        ...
        RSDK.SetSpriteAnimation(Ring->aniFrames, 0, &entity->animator, true, 0);
    }
    else {
...
void Ring_FakeLoseRings(Vector2 *position, int32 ringCount, uint8 drawOrder)
{
    ...
    for (int32 r = 0; r < ringCount2; ++r) {
        ...
        RSDK.SetSpriteAnimation(Ring->aniFrames, 1, &ringGrow->animator, true, 0);
        angle += 16;
...
void Ring_EditorLoad(void)
{
    ...
    RSDK_ENUM_VAR("None", RING_MOVE_NONE);
