...
typedef struct {
    ...
    int(*FetchLeaderboard)(uint8, uint8, int, int, int, int);
    int (*LeaderboardStatus)(void);
    int (*LeaderboardEntryCount)(void);
    LeaderboardEntry *(*LeaderboardReadEntry)(int);
    void(*TrackActClear)(uint8, uint8, uint8, int, int, int);
    void(*TrackTAClear)(int, int, int, int);
    void (*TrackEnemyDefeat)(uint8 zoneID, uint8 actID, uint8 playerID, int entityX, int entityY);
...
int APICallback_FetchLeaderboardData(uint8 a1, uint8 a2, int a3, int a4, int a5, bool32 isUser);
