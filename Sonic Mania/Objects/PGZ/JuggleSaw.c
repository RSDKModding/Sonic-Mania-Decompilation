    ...
    if (!SceneInfo->inEditor) {
        if (data) {
            ...
            switch (self->direction) {
                case FLIP_NONE: RSDK.SetSpriteAnimation(JuggleSaw->aniFrames, self->hasSaw == JSAW_HAS_SAW, &self->animator, true, 0); break;
                case FLIP_X:
                    self->direction = FLIP_Y;
                    RSDK.SetSpriteAnimation(JuggleSaw->aniFrames, self->hasSaw == JSAW_HAS_SAW, &self->animator, true, 0);
                    break;
                case FLIP_Y:
                    self->direction = FLIP_NONE;
                    RSDK.SetSpriteAnimation(JuggleSaw->aniFrames, 4 - (self->hasSaw != JSAW_HAS_SAW), &self->animator, true, 0);
                    break;
                case FLIP_XY:
...
void JuggleSaw_StageLoad(void)
{
    ...
    JuggleSaw->hitboxFriend.left   = -512;
    JuggleSaw->hitboxFriend.top    = -512;
    JuggleSaw->hitboxFriend.right  = 512;
    JuggleSaw->hitboxFriend.bottom = 512;
            ...
            if (!self->friends[0] || self->friends[0]->objectID != self->objectID) {
                ...
                foreach_active(JuggleSaw, newFriend)
                {
                    if (newFriend != self && newFriend->hasSaw == JSAW_NO_SAW && newFriend->setID == self->setID
                        ...
                        if (RSDK.CheckObjectCollisionTouchBox(newFriend, &JuggleSaw->hitboxBadnik, self, &JuggleSaw->hitboxFriend))
...
void JuggleSaw_StateCrab_ThrowSaw(void)
{
    ...
    if (animator->frameID == 3) {
        ...
        if (reciever->objectID == JuggleSaw->objectID) {
            ...
            RSDK.SetSpriteAnimation(JuggleSaw->aniFrames, self->spawnDir >= FLIP_Y ? 4 : 1, animator, true, 0);
            self->state = JuggleSaw_StateCrab_Handle;
        }
        self->friends[0] = NULL;
    }
    if (animator->frameID == animator->frameCount - 1) {
        RSDK.SetSpriteAnimation(JuggleSaw->aniFrames, self->spawnDir >= FLIP_Y ? 3 : 0, animator, true, 0);
        self->state = JuggleSaw_StateCrab_Handle;
    }
    JuggleSaw_CheckPlayerCollisions();
    JuggleSaw_CheckOffScreen();
...
void JuggleSaw_StateSaw_Handle(void)
{
    ...
    Hitbox *grabbox           = &JuggleSaw->hitboxGrabV;
    if (reciever->spawnDir >= FLIP_Y)
        grabbox = &JuggleSaw->hitboxGrabH;
    ...
    // collectively.
    // reciever->direction = oldDir; if (RSDK.CheckObjectCollisionTouchBox(reciever, grabbox, self, &JuggleSaw->hitboxSaw)) {

    bool32 collided     = RSDK.CheckObjectCollisionTouchBox(reciever, grabbox, self, &JuggleSaw->hitboxSaw);
    ...
    if (collided) {
        ...
        RSDK.SetSpriteAnimation(JuggleSaw->aniFrames, reciever->spawnDir >= FLIP_Y ? 4 : 1, &reciever->animator, true, 0);
        destroyEntity(self);
    }
