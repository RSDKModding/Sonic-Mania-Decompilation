    ...
    if (!SceneInfo->inEditor) {
        ...
        switch (self->type) {
            case TVVAN_SATELLITE_NE_SW:
                ...
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw2_High;
            ...
            case TVVAN_SATELLITE_SE_NW:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw3_High;
            ...
            case TVVAN_SATELLITE_NW_SW:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw4_High;
                self->field_C4[0]                        = 0x20;
                self->field_C4[1]                        = 0x20;
            ...
            case TVVAN_SATELLITE_NE_SE:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw5_High;
            ...
            case TVVAN_SATELLITE_NE_NW:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw6_High;
                self->field_C4[0]                        = 0x20;
                self->field_C4[1]                        = 0x20;
                self->field_CC[0]                        = 0x60;
                self->field_CC[1]                        = 0x60;
            ...
            case TVVAN_SATELLITE_SE_SW:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw7_High;
                self->field_C4[0]                        = 0xE0;
                self->field_C4[1]                        = 0xE0;
                self->field_CC[0]                        = 0xA0;
                self->field_CC[1]                        = 0xA0;
            ...
            case TVVAN_SATELLITE_SW_N:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw8_High;
            ...
            case TVVAN_SATELLITE_NW_S:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw9_High;
                self->field_C4[0]                        = 0x20;
                self->field_C4[1]                        = 0x20;
            ...
            case TVVAN_SATELLITE_SE_N:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw10_High;
                self->field_C4[0]                        = 0xA0;
                self->field_C4[1]                        = 0xA0;
            ...
            case TVVAN_SATELLITE_NE_S:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 6, &self->animator1, true, 0);
                self->drawOrder                     = Zone->drawOrderHigh;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw11_High;
            ...
            case TVVAN_EXITTV:
                ...
                self->stateDraw[Zone->drawOrderLow] = TVVan_StateDraw14_High;
                self->updateRange.x                = 0x1000000;
                self->updateRange.y                = 0x1000000;
                self->state                             = TVVan_State14_Unknown1;
                break;
            case TVVAN_TRANSPORT:
                RSDK.SetSpriteAnimation(TVVan->aniFrames, 10, &self->animator5, true, 0);
                self->active                        = ACTIVE_NORMAL;
                self->visible                       = 0;
                self->drawOrder                     = Zone->drawOrderHigh;
                self->state                              = TVVan_State15_Unknown1;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw15_High;
                break;
            default:
                self->drawOrder = Zone->drawOrderLow;
                self->state          = TVVan_State_Unknown1;
                if (self->type == TVVAN_VAN_RIGHT)
                    self->direction = FLIP_X;
                self->drawPos                            = self->position;
                self->drawPos2                           = self->position;
                self->stateDraw[Zone->drawOrderLow]  = TVVan_StateDraw_Low;
                self->stateDraw[Zone->drawOrderHigh] = TVVan_StateDraw_High;
...
void TVVan_StateDraw_Low(void)
{
    ...
    RSDK.DrawSprite(&self->animator1, 0, false);
    RSDK.DrawSprite(&self->animator5, 0, false);
...
void TVVan_State_Unknown2(void)
{
    ...
    if (self->animator4.frameID >= self->animator4.frameCount - 1) {
        self->state = TVVan_State_Unknown3;
        ...
        if (self->direction) {
            EntityTVVan *van = CREATE_ENTITY(TVVan, intToVoid(TVVAN_TRANSPORT), self->position.x + 0x80000, self->position.y - 0x420000);
...
void TVVan_State15_Unknown1(void)
{
    ...
    if (++self->timer == 20) {
        self->timer = 0;
        self->state = TVVan_State15_Unknown2;
    }
}
    ...
    if (Player_CheckValidState(player)) {
        foreach_active(TVVan, van)
        {
            if (self != van && self->nextNode != (Entity *)van) {
                if (van->type && van->type < TVVAN_TRANSPORT) {
                    ...
                    if (rx * rx + ry * ry < 0x800) {
                        ...
                        if (van->type == TVVAN_EXITTV) {
                            ...
                            self->state                      = TVVan_State15_Unknown5;
                            self->timer                      = 0;
                            self->direction                  = FLIP_NONE;
                        }
                        else {
                            ...
                            self->state = TVVan_State15_Unknown3;
                        }
                        foreach_break;
                    }
                }
...
void TVVan_State15_Unknown3(void)
{
    ...
    if (Player_CheckValidState(player)) {
        ...
        RSDK.PlaySfx(TVVan->sfxSatellite2, false, 255);
        self->state = TVVan_State15_Unknown4;
    }
    else {
        destroyEntity(self);
    }
...
void TVVan_State14_Unknown2(void)
{
    ...
    if (self->scale.x >= 0x200) {
        ...
        self->state             = TVVan_State14_Unknown3;
        self->stateDraw[Zone->drawOrderLow] = TVVan_StateDraw14_High;
        self->active       = ACTIVE_NORMAL;
    }
    else {
