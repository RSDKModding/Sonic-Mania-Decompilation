...
void LightBulb_Create(void *data)
{
    ...
    self->drawGroup     = (self->priority != LIGHTBULB_PRIO_HIGH) ? Zone->objectDrawLow : Zone->objectDrawHigh;
    self->visible       = true;
    self->alpha         = 0x100;
...
void LightBulb_State_CheckPlayerCollisions(void)
{
    ...
    foreach_active(Player, player)
    {
        if (player->animator.animationID != ANI_HURT && Player_CheckCollisionTouch(player, self, &LightBulb->hitboxLightBulb)) {
            ...
            CREATE_ENTITY(Explosion, INT_TO_VOID(EXPLOSION_BOSSPUFF), self->position.x, self->position.y - 0x100000)->drawGroup = Zone->objectDrawHigh;
