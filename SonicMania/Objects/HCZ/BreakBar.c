            ...
            for (int32 i = 0; i < self->length + 2; ++i) {
                ...
                if (self->orientation != BREAKBAR_V) {
                    ...
                    if (len > abs(debris->position.x - player->position.x))
                        debris->velocity.y +=
                            (2 * (player->velocity.y > 0) - 1) * (maxVal(len - abs(debris->position.x - player->position.x), 0) >> 5);
                    ...
                    if (len > abs(debris->position.y - player->position.y))
                        debris->velocity.x +=
                            (2 * (player->velocity.x > 0) - 1) * (maxVal(len - abs(debris->position.y - player->position.y), 0) >> 5);
